// Rota de Debug Temporária (Útil para pegar o Hash da nova chave)
Route::get('/debug-offline-hash', function () {
$key = \App\Models\ApiKey::where('software_id', 1) // AJUSTE SE O ID MUDAR
->get()
->filter(fn($k) => in_array('offline_activation', $k->scopes ?? []))
->last(); // Pega a ULTIMA criada

if (!$key) return 'Nenhuma chave Offline encontrada para Software ID 1';

return [
'software_id' => $key->software_id,
'key_hash_in_db' => $key->key_hash,
'delphi_should_use_this_secret' => $key->key_hash
];
});
